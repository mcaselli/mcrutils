...
`load_calendars()` is as a convenience wrapper around
`bizdays::load_quantlib_calendars()` that saves you from having to specify the
start and end date before loading the quantlib calendars--you just provide a
vector of dates and it will cover the full years spanned by those dates.
...
load_calendars(
  ...
  calendars = unique(sales$calendar)
)
...
# peek at the result
bizday_lookup |>
  filter(date >= ymd("2022-06-01")) |> # starting on a Sunday in June
  head()
```
