...
`accounts_by_status()` categorizes accounts into various statuses (active, new,
returning, temporarily lost, regained and terminally lost) as well as a running
list of cumulative accounts, over specified time periods (monthly or quarterly).
This is useful for understanding customer retention and churn.
...
accounts_by_status(example_sales$account_id, example_sales$order_date)
```
...
quarterly analysis is also supported by setting `by = "quarter"`

```{r accounts_by_status_quarterly}
accounts_by_status(example_sales$account_id, example_sales$order_date,
                   by = "quarter")
```
