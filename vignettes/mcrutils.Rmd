...
`mcrutils` includes an example dataset `example_sales` with 50 orders from 10
accounts over the year 2022.

```{r}
head(example_sales)
```

using this data, we can see how many accounts were new, returning, lost, or
regained each quarter

```{r accounts_by_status_example}
example_sales |> 
  accounts_by_status(account_id, order_date, by="quarter")
```

you can get the count of accounts in each status category by setting
`with_counts = TRUE`

```{r accounts_by_status_with_counts}
example_sales |>
  accounts_by_status(account_id, order_date,
    by = "quarter", with_counts = TRUE
  )
```


Here's how you might visualize the quarterly account status counts over time.
Showing the lost accounts as a negative value helps de-clutter the picture and
as well as helps perception by encoding values above the axis as "good" and
below as "bad" (assuming we don't want to lose customers). We can use color to
help as well (blues/greens: good, reds: bad).

```{r manual_accounts_plot, fig.width=7, fig.height=4}
library(ggplot2)
library(dplyr, warn.conflicts = FALSE)
library(tidyr)
...
before the end of the final time period, the final period will be shown with
dashed lines to indicate that the data may be incomplete.


```{r plot_accounts_1, fig.width=7, fig.height=4}
example_sales |> 
  plot_accounts_by_status(account_id, order_date, by="quarter")
```

You can suppress the dashed lines for incomplete periods with with `force_final_period_complete = TRUE`, and exclude the cumulative line with `include_cumulative = FALSE`.

```{r plot_accounts_2, fig.width=7, fig.height=4}
example_sales |> 
...
How were the year-to-date sales in the comparable period of 2024 and 2023?
