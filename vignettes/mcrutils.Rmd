...
as of a cutoff---typically the latest date in your data. Completeness is
...
`example_sales` has no orders after 2024-12-20, so as of that date December 2024
is still in progress while earlier months are complete:
...
Mapping the completeness flag to `alpha` and adding `scale_alpha_logical()` fades
the in-progress period so it reads as provisional:
...
When you would rather drop the partial period than fade it---for example before a
year-over-year comparison---`filter_complete_periods()` keeps only the rows whose
period is complete. By default it measures completeness against the latest date
in the column, so you never discard a period the data already covers in full:
