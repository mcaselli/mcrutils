...
Group the orders to monthly totals and flag each month with
`period_is_complete()`. With no `as_of`, completeness is judged as of the current
date, so for this historical dataset every month---including the partial
December, whose last order landed on the 20th---reads complete:
...
example_sales |>
  ...
  mutate(complete = period_is_complete(month, "month")) |>
  tail()
```

That is rarely what you want for a report on historical data. Pass an
`as_of`---typically the latest date you actually have---to judge completeness as
of when the data was current, and optionally a national `calendar` so holidays,
not just weekends, count as non-working days. Now December is correctly flagged
incomplete:
