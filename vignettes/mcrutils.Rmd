...
# make some orders data
n_orders <- 6000
set.seed(123)
orders <- tibble(
  order_date = sample(
    seq(as.Date("2024-01-01"), as.Date("2025-12-31"), by = "day"),
    n_orders,
    replace = TRUE
  ),
  units = rpois(n_orders, lambda = 3) + 1,
  market = sample(c("UnitedStates", "UnitedKingdom"), n_orders, replace = TRUE)
) |>
  arrange(order_date, market) |>
  mutate(order_id = row_number()) |>
  relocate(order_id)

# make a lookup table for business day of month and quarter

## first load the relevant calendars
load_calendars(
  dates = orders$order_date,
  calendars = unique(orders$market)
)

bizday_lookup <- tibble(
  # make a row for each date in the years spanned by the orders data
  date = seq(
    from = lubridate::floor_date(min(orders$order_date), "month"),
    to = lubridate::ceiling_date(max(orders$order_date), "month") - 1,
    by = "day"
  )
) |>
  # cross with each market
  tidyr::expand_grid(market = unique(orders$market)) |>
  mutate(
    # calculate the bizday of month and quarter for each date in each market
    bizday_of_month = purrr::pmap_int(
      list(.data$date, .data$market),
      \(date, market) {
        bizday_of_period(date, market, period = "month")
      }
    ),
    bizday_of_quarter = purrr::pmap_int(
      list(.data$date, .data$market),
      \(date, market) {
        bizday_of_period(date, market, period = "quarter")
      }
    )
  )

bizday_lookup |>
  filter(date >= as.Date("2025-06-02")) |> # a Sunday
  head()
```  

Then join to your orders data:

```{r}
orders_with_bizday <- orders |>
  left_join(bizday_lookup, by = c("order_date" = "date", "market" = "market"))
head(orders_with_bizday)
```

Now we can make a cumulative sum of units ordered by business day of month. 
Let's imagine it's mid-June 2025, and we want to see how orders are tracking against the prior year.

```{r}
orders_with_bizday |>
  filter(order_date < ymd("2025-06-19")) |>
  filter(month(order_date) == 6) |>
  group_by(year = year(order_date), bizday_of_month) |>
  summarise(units_ordered = sum(units), .groups = "drop") |>
  group_by(year) |>
