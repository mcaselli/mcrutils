...
Here we'll calculate the 1- and 2-year CAGR for revenue by market over a 3-year 
period.
```{r}
sales <- data.frame(
...
These decimal values are often not appropriate with count data,
so we provide `scale_x_integer()` and `scale_y_integer()`,
which set breaks with `floor(pretty())` and therefore guarantee
integer break labels.

```{r integer-scale-plots}
