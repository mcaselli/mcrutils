...
`accounts_by_status()` takes order data (account IDs and n order dates) and
categorizes accounts into various statuses (new, returning, temporarily lost,
terminally lost, and regained) over monthly or quarterly time periods. This is
useful for understanding customer retention and churn. (counts of accounts in
each status category can be included as well; set `with_counts = TRUE`).
