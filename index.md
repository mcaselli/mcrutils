...
takes order data (account IDs and order dates) and categorizes accounts
into different statuses (new, returning, temporarily lost, regained,
terminally lost) based on their order behavior in each time interval
(months, weeks, quarters, etc are supported).
...
monthly, quarterly) between two dates, using calendars from `QuantLib`
...
is a one-line function that creates a
[`DT::datatable`](https://rdrr.io/pkg/DT/man/datatable.html) object from
a data frame or tibble. It includes buttons to copy or download the
...
the data type and column names. See
[`vignette("mcrutils")`](https://mcaselli.github.io/mcrutils/articles/mcrutils.md)
for more examples, including how to specify the set of strings that flag
a column as percentage or currency.
...
![auto_dt()
output](%5Cman%5Cfigures%5CREADME-manual_screenshot_auto_dt.png)
...
generates similar labels for quarterly date breaks, labeling every
quarter, but only including the year when it changes from the previous
label.
...
![](reference/figures/README-automatic-quarterly-breaks-1.png)

By default,
[`breaks_quarters()`](https://mcaselli.github.io/mcrutils/reference/breaks_quarters.md)
tries to return a reasonable number of breaks over a wide range of
dates, down-sampling to semesters and years as needed. See
[`vignette("mcrutils")`](https://mcaselli.github.io/mcrutils/articles/mcrutils.md)
for more examples.
